...
# for kubernetes Stateful pods
if [[ $INDEX_FROM_HOSTNAME == 'true' ]]; then
...
if [[ $ADVERTISED_LISTENERS_FROM_HOSTNAME == 'true' ]]; then
  export KAFKA_LISTENERS="PLAINTEXT://:9092,EXTERNAL://:9093,SASL_PLAINTEXT://:9094"
  export KAFKA_ADVERTISED_LISTENERS="PLAINTEXT://$KUBERNETES_POD-$HOST_INDEX.$KUBERNETES_SERVICE:$KAFKA_PORT",EXTERNAL://${KAFKA_EXTERNAL_LISTENER_HOST:localhost}:3002$HOST_INDEX,SASL_PLAINTEXT://${KAFKA_EXTERNAL_LISTENER_HOST:localhost}:3003$HOST_INDEX
  export KAFKA_LISTENER_SECURITY_PROTOCOL_MAP="PLAINTEXT:PLAINTEXT,EXTERNAL:PLAINTEXT,SSL:SSL,SASL_PLAINTEXT:SASL_PLAINTEXT,SASL_SSL:SASL_SSL"
  echo "$KAFKA_ADVERTISED_LISTENERS"
fi

# Try and configure minimal settings or exit with error if there isn't enough information
if [[ -z "$KAFKA_ADVERTISED_HOST_NAME$KAFKA_ADVERTISED_LISTENERS" ]]; then
